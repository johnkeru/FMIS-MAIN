import { Alert, AlertTitle, Autocomplete, Box, Button, Card, CardContent, Fade, FormControl, FormControlLabel, FormLabel, Radio, RadioGroup, TextField, Typography } from '@mui/material';
...
const SignatoryCreate = () => {
    ...
    const selectedAmount = watch('amount', null);
    ...
    useEffect(() => {
        // BASIC
        if (transactionTypeValue && transactionTypeValue.responsibilityCenters[0].type === 'basic') {
            const responsibilityCenter = transactionTypeValue.responsibilityCenters[0];
            // setting up boxes
            const boxes = responsibilityCenter.boxes;
            // map boxes to find signatories(names)
            const updatedBoxes = boxes.map((boxInfo) => {
                const signatory = data.employees.find(emp => boxConditions(boxInfo, emp));
                return {
        ...
        // AMOUNT for 'amount' type
        else if (transactionTypeValue && transactionTypeValue.responsibilityCenters[0].type === 'amount' && !selectedAmount) {
            const amounts = transactionTypeValue.responsibilityCenters[0].amounts
            setShowAmounts(amounts)
            setValue('amount', amounts[0].amount);
        } else if (selectedAmount) {
            const amounts = transactionTypeValue.responsibilityCenters[0].amounts
            const foundSignatoriesForAmount = amounts.filter(amount => amount.amount === selectedAmount)[0]
            if (!foundSignatoriesForAmount) return
            const boxes = foundSignatoriesForAmount.boxes;
    ...
    }, [transactionTypeValue, data, register, selectedAmount]);

    useEffect(() => {
        // POSITION
        if (transactionTypeValue && transactionTypeValue.responsibilityCenters[0].type === 'position' && payeeValue) {
    ...
    // this second useEffect is for clearing form state when transaction type changes
    useEffect(() => {
        if (transactionTypeValue && transactionTypeValue.responsibilityCenters[0].type !== 'amount') {
            setValue('amount', '');
            setShowAmounts([]);
        }
        if (transactionTypeValue && transactionTypeValue.responsibilityCenters[0].type === 'position' && !payeeValue) {
        ...
        <>
            ...
            <Box component="form" onSubmit={handleSubmit(onSubmit)} sx={{ p: { xs: 2, sm: 3 } }}>
                ...
                <Card sx={{ p: 2, boxShadow: 3, mb: 3 }}>
                    <CardContent>
                        {
                            showAmounts.length === 0 ? undefined : (
                                <Controller
                                    name='amount'
                                    control={control}
                                    defaultValue={showAmounts[0].amount} // Set the default value here
                                    ...
                                    render={({ field }) => (
                                        <FormControl {...field}>
                                            ...
                                            <RadioGroup
                                                ...
                                                onChange={field.onChange} // Update value when a new option is selected
                                            >
                                                {
                                                    showAmounts.map(amount => (
                                                        <FormControlLabel
                                                            key={amount._id}
                                                            value={amount.amount}
                                                            control={<Radio />}
