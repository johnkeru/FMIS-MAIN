...
const LinkTo = ({ icon, name, link, subLinks, isAllow = false }) => {
  ...
  return (
    <>
      {isAllow || (subLinks && subLinks.find((p) => p.isAllow)) ? (
        <ListItem
          ...
          sx={{
            cursor: "pointer",
            bgcolor: isActive ? "rgba(250,250,250,.15)" : "transparent",
            "&:hover": {
              bgcolor: "rgba(250,250,250,.15)",
            },
          }}
        >
          ...
          {!subLinks ? undefined : !open ? (
            <IoMdArrowDropdown style={{ color: "white", fontSize: "1.3rem" }} />
          ) : (
            <IoMdArrowDropup style={{ color: "white", fontSize: "1.3rem" }} />
          )}
        </ListItem>
      ) : undefined}
